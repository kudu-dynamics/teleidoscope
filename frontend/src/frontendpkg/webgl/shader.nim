## shader.nim
##
## WebGL shaders and texture from ngraph.pixel.
...
proc createParticleMaterial(): JsObject =
  ...
  return jsNew three.ShaderMaterial(js{
    "uniforms": uniforms,
...
proc createEdgeMaterial(): JsObject =
  result = jsNew three.LineBasicMaterial(js{
    "vertexColors": three.VertexColors
  })

proc makeMesh*(geometry: JsObject, mesh_type: string): JsObject =
  case mesh_type
  of "nodes":
    result = jsNew three.Points(geometry, createParticleMaterial())
  of "edges":
    result = jsNew three.LineSegments(geometry, createEdgeMaterial())
  else:
    raise newException(ValueError, fmt"invalid mesh_type: {mesh_type}")
  result.name = mesh_type.cstring
